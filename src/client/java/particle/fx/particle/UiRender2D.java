...
final class UiRender2D {
	...
	static void roundedRect(DrawContext context, int x, int y, int width, int height, int radius, int color) {
		...
		drawSurface(context, new SurfaceKey(SurfaceType.RECT, width, height, radius, color, color, 0, 0), x, y);
	}
	...
	static void gradientRoundedRect(DrawContext context, int x, int y, int width, int height, int radius, int topColor, int bottomColor) {
		...
		drawSurface(context, new SurfaceKey(SurfaceType.GRADIENT, width, height, radius, topColor, bottomColor, 0, 0), x, y);
	}
	...
	static void border(DrawContext context, int x, int y, int width, int height, int radius, int color) {
		...
		drawSurface(context, new SurfaceKey(SurfaceType.BORDER, width, height, radius, color, color, 0, 0), x, y);
	}
	...
	private static int pixel(SurfaceKey key, float px, float py) {
		return switch (key.type) {
			case RECT -> applyCoverage(key.topColor, roundedCoverage(px, py, key.width, key.height, key.radius));
			case GRADIENT -> applyCoverage(lerpColor(key.topColor, key.bottomColor, py / Math.max(1.0F, key.height - 1.0F)),
					roundedCoverage(px, py, key.width, key.height, key.radius));
	...
	private static int borderPixel(SurfaceKey key, float px, float py) {
		float outer = roundedCoverage(px, py, key.width, key.height, key.radius);
		float inner = roundedCoverage(px - 1.0F, py - 1.0F, key.width - 2, key.height - 2, Math.max(0, key.radius - 1));
