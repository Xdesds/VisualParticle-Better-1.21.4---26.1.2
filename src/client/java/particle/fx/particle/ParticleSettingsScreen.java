...
public class ParticleSettingsScreen extends Screen {
	...
	private static final int LANGUAGE_W = 50;
	private static final int LANGUAGE_H = 16;
	private static final Identifier FLAG_RU = Identifier.of(ParticleFxMod.MOD_ID, "textures/gui/flag_ru.png");
	private static final Identifier FLAG_GB = Identifier.of(ParticleFxMod.MOD_ID, "textures/gui/flag_gb.png");
	...
	private void renderWindow(DrawContext context, int mouseX, int mouseY) {
		UiRender2D.softShadow(context, x, y, windowW, windowH, 7, 16, 0xAA000000);
		UiRender2D.blurredBackdrop(context, x, y, windowW, windowH, 7, 0xF20A0B10);
		UiRender2D.gradientRoundedRect(context, x, y, SIDEBAR_W, windowH, 7, 0xF412131B, 0xF007080C);
		UiRender2D.gradientRoundedRect(context, contentX, y, contentW, windowH, 7, 0xF00E1018, 0xDA08090E, UiRender2D.CORNERS_RIGHT);
		UiRender2D.gradientRoundedRect(context, contentX, y, contentW, TOP_H + 1, 7, 0xF00E1018, 0xEA090A0F, UiRender2D.CORNER_TOP_RIGHT);
		UiRender2D.roundedRect(context, contentX, y + TOP_H, contentW, contentH, 0, 0xDA08090E);
		UiRender2D.roundedRect(context, contentX, y + windowH - FOOTER_H, contentW, FOOTER_H, 7, 0xDA08090E, UiRender2D.CORNER_BOTTOM_RIGHT);
	...
	private void renderSidebar(DrawContext context, int mouseX, int mouseY) {
		...
		for (Tab value : Tab.values()) {
			...
			drawText(context, t(value.label), x + 17, tabY + 6, selected ? 0xFFFFFFFF : MUTED);
			tabY += 22;
		}

		int statusY = y + windowH - 50;
		UiRender2D.pill(context, x + 10, statusY, SIDEBAR_W - 20, 17, settings.enabled ? 0x88215F42 : 0x884A252A);
		drawText(context, settings.enabled ? t("Enabled") : t("Disabled"), x + 20, statusY + 5, settings.enabled ? GOOD : DANGER);
		UiRender2D.pill(context, x + 10, statusY + 22, SIDEBAR_W - 20, 17, 0xAA11131A);
		drawText(context, option(settings.colorPreset), x + 18, statusY + 27, DIM);
	}

	private void renderTopBar(DrawContext context, int mouseX, int mouseY) {
		String title = t(tab.title);
		String subtitle = t(tab.subtitle);
		drawText(context, title, contentX + 10, y + 8, TEXT);
		drawText(context, subtitle, contentX + 10 + textWidth(title) + 8, y + 9, MUTED);
		renderLanguageButton(context, mouseX, mouseY);
		String glow = option(settings.glowMode);
		int languageX = languageButtonX();
		int chipW = Math.max(58, textWidth(glow) + 16);
		int chipX = languageX - chipW - 7;
		UiRender2D.pill(context, chipX, y + 8, chipW, 15, 0x99151720);
		drawCenteredText(context, glow, chipX + chipW / 2, y + 12, DIM);
	}
	...
	private void renderFooter(DrawContext context, int mouseX, int mouseY) {
		...
		int bx = contentX + 10;
		drawFooterButton(context, mouseX, mouseY, bx, fy, 58, 17, t("Save"), primaryColor());
		drawFooterButton(context, mouseX, mouseY, bx + 64, fy, 58, 17, t("Reset"), DANGER);
		drawFooterButton(context, mouseX, mouseY, bx + 128, fy, 58, 17, t("Done"), GOOD);
		drawText(context, System.currentTimeMillis() - lastSaveTime < 1300L ? t("saved") : t("autosave"), x + windowW - 64, fy + 5, DIM);
	}
	...
	private void rebuildControls() {
		...
		if (tab == Tab.GENERAL) {
			cy = addCard(cy, cx, cw, "Main", List.of(
					...
					mode("Color preset", () -> option(settings.colorPreset), colorPresetOptions()),
					mode("Particle shape", () -> option(settings.particleMode), particleModeOptions(false)),
					mode("Glow mode", () -> option(settings.glowMode), glowModeOptions())
			));
			...
			cy = addCard(cy, cx, cw, "World", List.of(
					...
					mode("World shape", () -> option(settings.worldMode), particleModeOptions(true)),
	...
	public boolean mouseClicked(Click click, boolean doubled) {
		...
		if (hovered(mouseX, mouseY, languageButtonX(), languageButtonY(), LANGUAGE_W, LANGUAGE_H)) {
			settings.uiLanguage = settings.uiLanguage.next();
			saveSettings();
			openDropdown = null;
			openColor = null;
			return true;
	...
	private List<Option> colorPresetOptions() {
		...
		for (ParticleSystem.ColorPreset preset : ParticleSystem.ColorPreset.values()) {
			options.add(new Option(option(preset), () -> {
				settings.markCustom();
	...
	private List<Option> particleModeOptions(boolean world) {
		...
		for (Particle3D.ParticleMode mode : Particle3D.ParticleMode.values()) {
			options.add(new Option(option(mode), () -> {
				settings.markCustom();
				if (world) {
	...
	private List<Option> presetOptions() {
		List<Option> options = new ArrayList<>();
		options.add(new Option(t("Custom"), () -> {
			settings.markCustom();
			saveSettings();
		}));
		options.add(new Option("PvP", () -> applyPreset(ParticleSystem.Preset.PVP)));
		options.add(new Option(t("Minimal"), () -> applyPreset(ParticleSystem.Preset.MINIMAL)));
		options.add(new Option(t("Cinematic"), () -> applyPreset(ParticleSystem.Preset.CINEMATIC)));
		for (String presetName : ParticleConfig.getCustomPresetNames()) {
	...
	private List<PresetOption> getPresetOptions() {
		...
		options.add(PresetOption.builtIn(ParticleSystem.Preset.MINIMAL, t("Minimal")));
		options.add(PresetOption.builtIn(ParticleSystem.Preset.CINEMATIC, t("Cinematic")));
		for (String presetName : ParticleConfig.getCustomPresetNames()) {
	...
	private void openPresetNameScreen() {
		if (client != null) {
			client.setScreen(new PresetNameScreen(this, t("Create Preset"), t("Create"), settings.uiLanguage, this::createPreset));
		}
	}
	...
	private String activePresetLabel() {
		...
		return switch (settings.preset) {
			case CUSTOM -> t("Custom");
			case PVP -> "PvP";
			case MINIMAL -> t("Minimal");
			case CINEMATIC -> t("Cinematic");
		};
	}
	...
	private String t(String key) {
		return settings.uiLanguage.text(key);
	}

	private String option(Enum<?> value) {
		return settings.uiLanguage.option(value);
	}
	...
	private void drawCenteredText(DrawContext context, String text, int centerX, int y, int color) {
		drawText(context, text, centerX - textWidth(text) / 2, y, color);
	}
	...
	private void drawFooterButton(DrawContext context, int mouseX, int mouseY, int x, int y, int w, int h, String text, int color) {
		...
		drawCenteredText(context, text, x + w / 2, y + 5, 0xFFFFFFFF);
	}

	private void renderLanguageButton(DrawContext context, int mouseX, int mouseY) {
		int lx = languageButtonX();
		int ly = languageButtonY();
		boolean hover = hovered(mouseX, mouseY, lx, ly, LANGUAGE_W, LANGUAGE_H);
		UiRender2D.roundedRect(context, lx - 1, ly - 1, LANGUAGE_W + 2, LANGUAGE_H + 2, 7,
				hover ? UiRender2D.withAlpha(primaryColor(), 72) : 0x24FFFFFF);
		UiRender2D.gradientRoundedRect(context, lx, ly, LANGUAGE_W, LANGUAGE_H, 7,
				hover ? UiRender2D.withAlpha(primaryColor(), 180) : 0xD212141D,
				hover ? UiRender2D.withAlpha(secondaryColor(), 170) : 0xD20B0D13);
		Identifier flag = settings.uiLanguage == UiLanguage.RU ? FLAG_RU : FLAG_GB;
		context.drawTexture(RenderPipelines.GUI_TEXTURED, flag, lx + 5, ly + 3, 0.0F, 0.0F, 16, 10, 16, 10, 16, 10);
		drawText(context, settings.uiLanguage.code(), lx + 26, ly + 5, 0xFFFFFFFF);
	}

	private int languageButtonX() {
		return x + windowW - LANGUAGE_W - 10;
	}

	private int languageButtonY() {
		return y + 7;
	}
	...
	private final class CardControl extends Control {
		...
		protected void render(DrawContext context, int mouseX, int mouseY) {
			UiRender2D.softShadow(context, x, y, width, height, 7, 8, 0x55000000);
			UiRender2D.roundedRect(context, x - 1, y - 1, width + 2, height + 2, 7, 0x24FFFFFF);
			UiRender2D.gradientRoundedRect(context, x, y, width, height, 7, 0xC813141C, 0xB70C0D13);
			drawText(context, t(label), x + 8, y + 8, TEXT);
	...
	private final class ToggleControl extends SettingControl {
		...
		protected void render(DrawContext context, int mouseX, int mouseY) {
			...
			drawText(context, t(label), x, y + 5, TEXT);
	...
	private final class CycleControl extends SettingControl {
		...
		protected void render(DrawContext context, int mouseX, int mouseY) {
			...
			int valueW = Math.min(96, Math.max(54, textWidth(value) + 14));
			UiRender2D.pill(context, x + width - valueW, y + 3, valueW, 12, 0xBB151720);
			drawCenteredText(context, value, x + width - valueW / 2, y + 5, primaryColor());
		}
	...
	private final class SliderControl extends SettingControl {
		...
		protected void render(DrawContext context, int mouseX, int mouseY) {
			...
			drawText(context, t(label), x, y + 1, TEXT);
			drawText(context, value, x + width - textWidth(value), y + 1, MUTED);
	...
	private final class ButtonControl extends SettingControl {
		...
		protected void render(DrawContext context, int mouseX, int mouseY) {
			renderHighlight(context, mouseX, mouseY);
			String buttonText = t(actionLabel);
			drawText(context, t(label), x, y + 5, enabled.getAsBoolean() ? TEXT : DIM);
			int bw = Math.max(42, textWidth(buttonText) + 16);
			UiRender2D.pill(context, x + width - bw, y + 3, bw, 12, enabled.getAsBoolean() ? color : 0xFF252936);
			drawCenteredText(context, buttonText, x + width - bw / 2, y + 5, 0xFFFFFFFF);
		}
